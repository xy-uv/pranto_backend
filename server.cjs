...
app.post("/api/categories", async (req, res) => {
  ...
  if (await writeJsonFile(FILES.categories, categories)) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to save category" });
  }
});
...
app.put("/api/users/:id", async (req, res) => {
  ...
  const { email, name } = req.body;
  if (
    users.some((u, i) => i !== index && (u.email === email || u.name === name))
  ) {
    return res
      .status(400)
      .json({
...
app.post("/api/administrators", async (req, res) => {
  ...
  if (await writeJsonFile(FILES.administrators, administrators)) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to save administrator" });
  }
});
...
app.put("/api/administrators/:id", async (req, res) => {
  ...
  if (
    administrators.some(
      (a, i) => i !== index && (a.email === email || a.name === name)
    )
  ) {
    return res
      .status(400)
...
app.post("/api/purchases", async (req, res) => {
  ...
  if (await writeJsonFile(FILES.purchases, purchases)) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to save purchase" });
  }
});
...
app.post("/api/sales-with-warranty", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      saleId,
      message: "Sale with warranty recorded successfully",
    });
  } catch (error) {
    console.error("Error saving sale with warranty:", error);
    res
      .status(500)
      .json({ success: false, message: "Failed to save sale with warranty" });
  }
});
  ...
  try {
    ...
    const warrantyInfo = matchingSales.map((sale) => {
      ...
      return {
        ...
        daysRemaining: isWarrantyActive
          ? Math.ceil((warrantyEndDate - today) / (1000 * 60 * 60 * 24))
          : 0,
      };
    });
    ...
    res
      ...
      .json({
        success: false,
        message: "Failed to search warranty information",
      });
  }
});
...
app.post("/api/warranty/approve", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to approve warranty claim" });
  }
});
...
app.get("/api/warranty/approvals", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch warranty approvals" });
  }
});
...
app.get("/api/sales/daily/:date", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch daily sales" });
  }
});
...
app.get("/api/soldproducts", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch sold products" });
  }
});
...
app.post("/api/data-clear", async (req, res) => {
  try {
    ...
    const filesToClear = [
      ...
      {
        path: path.join(DATA_DIR, "revenue.json"),
        defaultValue: { totalRevenue: 0 },
      },
      { path: FILES.purchases, defaultValue: [] },
      {
        path: path.join(DATA_DIR, "inventory.json"),
        defaultValue: { totalProducts: 0 },
      },
      {
        path: path.join(DATA_DIR, "profit.json"),
        defaultValue: { netProfit: 0 },
      },
    ];
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to clear all data." });
  }
});
